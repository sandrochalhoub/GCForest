...
namespace primer {
  ...
  size_t DataSet::count() const { return example[0].count() + example[1].count(); }
  ...
  void DataSet::getContradictingFeatures(instance &x1, instance &x2, instance &buffer) {
    buffer = NOT(x2);
    // buffer = x2;
    // buffer >>= nFeature() / 2;
  ...
  std::ostream& DataSet::display_example(std::ostream &os, const instance e) const {
