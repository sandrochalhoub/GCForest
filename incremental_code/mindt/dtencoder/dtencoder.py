...
class DTEncoder(object):
    ...
    def __init__(self, data, options):
        ...
        # Louis # Use default fvmap so that the positive and negative labels would be deterministic
        outs = {k : int(v[1]) for k, v in self.data.fvmap.opp.items()}
        ...
        print("fvmap.opp: ", outs)
        # saving options
        self.options = options
        self.var2ids = {}
        # Louis 2020-02-20 - Add solver time limit in seconds
        self.time_limit = options.solver_time
...
#         # plt.show()
    ...
    def generate_formula(self, N, K=None):
        ...
        if self.time_limit <= 0:
            ...
            timer = Timer(self.time_limit, lambda solver: solver.interrupt(), [self.s])
            timer.start()
            res = self.s.solve_limited()
            timer.cancel()
    ...
    def generate_branch_clauses(self, N, K):
        """
            Louis 2020-02-26
            Generate clauses to restrict usage of features on certain branch (exp 2)
        """
        for i in range(N):
            # Pour chaque noeud
    ...
    def init_polarity_from_tree(tree, N, K):
        """
            Louis 2020-02-25
            [Unfinished] Function to set the polarity of a tree
        """
        for i in range(N):
            # get variable vi =
            vi = tree.map_node[i].is_leaf()

            for j in range(N):
                if i != j:
                    left_child = tree.map_node[i].left_child
                    lij = left_child != None and left_child.id == j

                    right_child = tree.map_node[j].right_child
                    rij = right_child != None and right_child.id == j

                    pji = lij or rij

            ci = tree.map_node[i].features_value == 1

            for r in range(K):
                fval = tree.map_node[i].features_value
                fid = self.name_feature.index(fval)
                print(fval, fid)
                arj = fid == r

        # TODO see if we can set polarity for u and d too
