...
class Data(object):
    ...
    def check_ds(self, dset, check_overlap, verb):
        ...
        for rule in dset.rules:
            ...
            if check_overlap:
                others = set(self.classes.iterkeys()).difference(set([label[1]]))
                for lb in others:
        ...
        if len(covered) != len(self.samps) or overlap:
            print('accuracy: {0:.2f}%'.format(100.0 * float(len(covered.difference(overlap))) / len(self.samps)))
