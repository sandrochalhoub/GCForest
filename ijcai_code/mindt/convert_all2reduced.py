...
def run(path_in_str):
    #print(path_in_str)
    if ("_" not in path_in_str):
        print(path_in_str)
        os.system("python src/mindt/feature_selection.py " +  path_in_str)
    #exit()

def convert_all2reduced():
    ...
    pathlist1 = Path(path_manu_bench).glob('**/*-un.csv')
    pathlist2 = Path(path_penn_bench).glob('**/*-un.csv')
