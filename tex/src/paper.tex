...
\SetKw{Break}{break}
	...
	\SetKwFunction{budalg}{Blossom}
	\SetKwFunction{dynprog}{DynProg}
	\SetKwFunction{dleight}{DL8.5}
	\SetKwFunction{olddleight}{DL8}
  ...
  \SetKwFunction{murtree}{Murtree}
	...
	\SetKwFunction{idthree}{ID3}
	\SetKwFunction{cfour}{C4.5}
	\SetKwFunction{cp}{CP}
	\SetKwFunction{binoct}{binOCT}
	\SetKwFunction{iti}{ITI}
	\SetKwFunction{bfswee}{Blossom ($3/4$)}
	\SetKwFunction{bfs}{Blossom}
	\SetKwFunction{bfss}{Blossom (pruning)}
	\SetKwFunction{bfsm}{Blossom (pruning)}
	\SetKwFunction{bfsh}{Blossom (with pruning)}
...
A \emph{branch} of a decision tree can therefore be identified to the ordered set of labels on the corresponding edges.
...
The solver DL8.5 is a dynamic programming algorithm for the minimum error bounded depth decision tree problem. It relies on the observation that given a feature test, the two resulting branches are independent subproblems. Algorithm~\ref{alg:dynprog} gives a high level view of DL8.5.
	...
	We can safely assume $\mdepth \leq \numfeat$ (otherwise all features can be tested on every branch) and $\mdepth \leq \log \numex$ (otherwise we could have one distinct branch per datapoint), and it is often assumed that
	 ...
	 Moreover, at each call, the data set must be split into two subsets. This takes time linear in the size of the data set. However, consider an ordered set of $\mdepth$ features and the $2^{\mdepth-1}$ branches testing these features in that order. Their corresponding data sets form a partition of the original data set. Therefore, the $\mdepth$-th split can done in $\Theta(\numex)$ time amortized over these $2^{\mdepth-1}$ branches.
...
In a nutshell the algorithm we propose (Algorithm~\ref{alg:bud}) works as follows:
...
% \medskip
		...
		\begin{theorem}
			...
			\begin{proof}[sketch]
				...
				Now suppose that for $\mdepth \leq d$ \dynprog and \budalg explore exactly the same set of branches: a recursive call of \dynprog ends on the branch $\abranch$ if and only if $\best[\abranch]$ is set in Line~\ref{line:best} of \budalg.
...
\begin{footnotesize}
		...
		\lnl{line:dive}\eIf{$\bud \neq \emptyset$}{
			...
			\lnl{line:backtrack}\While{$|\sequence| > 0$}{
				...
				\lnl{line:optimal}\eIf{$\dom[\abranch] \neq \emptyset$ \colorbox{yellow!50}{and $|\abranch|<\mdepth$}} {
					\lnl{line:fail} \HiLi \If{$\forall \abranch' \subseteq \abranch, \lb{\abranch',\abranch} < \best[\abranch']$} {
				...
				{
					\lnl{line:markoptimal}\HiLi $\opt[\abranch] \gets 1$\;
				}
				% \lElse {
