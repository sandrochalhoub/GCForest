...
Every algorithm was run until completion or until it reaches a time limit of one hour. We imposed a memory limit of 50GB on every algorithm.


We used a collection of 58 data sets formed by the union of the data sets from related work~\cite{narodytska2018learning,dl85,verwer2019learning}, to which we added extra data sets (\texttt{bank}, \texttt{titanic}, \texttt{surgical-deepnet} and \texttt{weather-aus}, as well as \texttt{mnist\_0}, \texttt{adult\_discretized}, \texttt{compas\_discretized} and \texttt{taiwan\_binarised}). Further description of the data sets as well as all expeimental results are given in appendix. For reason of space, we present aggregated results in this section.



\subsection{Optimizing trees}

We first compare our algorithm (\budalg) to state-of-the-art algorithms, \murtree~\cite{DBLP:journals/corr/abs-2007-12652} and \dleight~\cite{dl85}, as well as the best MIP (\binoct)~\cite{verwer2019learning} and CP (\cp)~\cite{verhaeghe2019learning} models, for computing optimal trees and proving optimality. 

The data sets in Table~\ref{tab:summaryaccsmall} are organized by their number of datapoints \numex and the size \numfeat of their feature set.
For each class, every method is run with an upper bound $\mdepth$ on the tree depth shown in the first column. 
We report for every class and every depth: the ratio of optimality proofs (opt.); the average classification error (error); and %the average accuracy (acc.), 
 %as well as 
...
For the number of optimality proofs, \budalg is comparable to \murtree.
It is slightly less efficient for  $\mdepth \leq 5$, but slightly more efficient for $\mdepth \geq 7$. The gap on shallow tree can be explained by \murtree's caching method, and because it puts less emphasis on trying to find good trees faster, but rather tries to exhaust the whole search space faster. The gap on deep trees can mostly be explained by the preprocessing technique whereby inconsistent datapoints are supressed: whereas \budalg can stop searching when the overll classification error reaches the number of inconsistent datapoints, \murtree must exhaust the search space. Despite what a quick look at Table~\ref{tab:summaryaccsmall} may suggests, both methods have similar speed. The large gaps are either due to the same phenomenon described above (when in favor or \budalg), or due to a single data set, namely \texttt{mnist\_0} (when in favor or \murtree).
Both algorithms find trees of similar qualities for $\mdepth \leq 5$, however, \budalg is significantly better for $\mdepth \geq 7$. 

All other methods are systematically outperformed. \cp has good results on very shallow trees ($\mdepth \leq 4$) but is ineffective for deeper tree. Indeed, the quality of the tree actually \emph{decreases} when $\mdepth$ increases over $4$! \dleight can also find optimal trees in most cases where the number of feature (\numfeat) is low, and for small values of $\mdepth$. When, $\numfeat$ grows, however, it often exceeds the memory limit of 50GB (whereas \budalg does not require more memory than the size of the data set). Finally, \binoct does not produce a single optimality proof and very often exceeds the memory limit.%\footnote{In the experiments in \cite{verwer2019learning} not all datapoints were used.}
...
Most toolkits for computing decision trees somehow try to balance size and accuracy. As discussed earlier, \budalg use the relatively standard approach to bound the maximum depth and searches for the tree with maximum accuracy within that limit. Other methods focus on size rather than depth. 
For instance, the algorithm \gosdt~\cite{NEURIPS2019_ac52c626} optimize a linear combination of the classification error and of the number of leaves. 

From \budalg, we designed a method to trade accuracy for size based on pruning. Given a decition tree of accuracy $\alpha$ and a target accuracy $\tau \leq \alpha$, the subtree of size $s_i$ and classification error $\alpha_i$ such that $\alpha_i/s_i$ is minimum is suppressed, as long as the overall accuracy remains larger than or equal to $\tau$.  We did not manage, unfortunately, to obtain a relevant comparison with \gosdt, because no setting of the regularization parameter enable us to obtain trees with more than a dozen leafs. Instead we experimented with \iti~\cite{}
...
\begin{scriptsize}%
\tabcolsep=2pt%
\input{src/tables/allfactors_smallsmall_3.tex}%
\end{scriptsize}%
\end{center}%
\caption{\label{tab:all31} Comparison with state of the art: $\numex<5000, \numfeat<250$, depth 3}%
\end{table}%
...
\begin{scriptsize}%
\tabcolsep=2pt%
\input{src/tables/allfactors_smallsmall_4.tex}%
\end{scriptsize}%
\end{center}%
\caption{\label{tab:all41} Comparison with state of the art: $\numex<5000, \numfeat<250$, depth 4}%
\end{table}%
...
\begin{scriptsize}%
\tabcolsep=2pt%
\input{src/tables/allfactors_smallsmall_5.tex}%
\end{scriptsize}%
\end{center}%
\caption{\label{tab:all51} Comparison with state of the art: $\numex<5000, \numfeat<250$, depth 5}%
\end{table}%
...
\begin{scriptsize}%
\tabcolsep=2pt%
\input{src/tables/allfactors_smallsmall_7.tex}%
\end{scriptsize}%
\end{center}%
\caption{\label{tab:all71} Comparison with state of the art: $\numex<5000, \numfeat<250$, depth 7}%
\end{table}%
...
\begin{scriptsize}%
\tabcolsep=2pt%
\input{src/tables/allfactors_smallsmall_10.tex}%
\end{scriptsize}%
\end{center}%
\caption{\label{tab:all101} Comparison with state of the art: $\numex<5000, \numfeat<250$, depth 10}%
\end{table}%
