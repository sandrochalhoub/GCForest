		...
		\lnl{line:dive}\eIf{$\bud \neq \emptyset$}{
			...
			\lnl{line:backtrack}\While{$|\sequence| > 0$}{
				...
				\eIf{$\dom[\abranch] \neq \emptyset$ \colorbox{yellow!50}{and $|\abranch|<\mdepth$}} {
					\HiLi \If{$\forall \abranch' \subseteq \abranch, \lb{\abranch',\abranch} < \best[\abranch']$} {
...
\begin{example}[Lower bound reasoning]
	...
	Figure~\ref{fig:lowerbound} shows a snapshot of the excution of \budalg. Leaves of the tree labelled by a dot (``.'') are buds of the tree, waiting for a feature test on that branch. In other words, we have $\bud=\{\{b,a\},\{b,\bar{a},c,\bar{g}\}\}$. Other nodes are labelled with their feature test, and with the values of $\best[\abranch]$ for the branch $\abranch$ ending on that node. When all subtrees of a branch $\abranch$ have been explored (hence $\opt[\abranch]=1$), this is marked by a ``$^*$''. 
			...
			\begin{forest}
				...
				[{$b:50$}
					[{$a:22$}, edge={very thick}, edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$b$}}
					 ...
					 [{$c:17$}, edge={very thick}, edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$\bar{a}$}}
