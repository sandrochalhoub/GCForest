...
This algorithm can be seen as an instance of the framework introduced in \cite{DBLP:journals/corr/abs-2007-12652}, tuned to have the best scalability and anytime behavior as possible.
...
$
\sum\limits_{x \in \allex}\min(\weight_{\bot}(x), weight_{\top}(x))
$.
...
A feature $\afeat$ is redundant if there exists a feature $\afeat'$ such that either: $\forall x \in \allex, \afeat \in x \iff \afeat' \in x$, or $\forall x \in \allex, \afeat \in x \iff \afeat' \in x$. We simply remove such redundant features. 
