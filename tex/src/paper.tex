...
% $2^{\features}$.
% It is associated a label function $\classlabel : 2^{\features} \mapsto \{\top,\bot\}$ such that:
% $$
...
A \emph{branch} of a decision tree we can therefore be identified to the set of labels on the corresponding edges.
...
\begin{itemize}
	...
	%
	%
	%  a feature $\afeat$ marked as \emph{available} for \abranch, add the pair $(\abranch,\afeat)$ to \sequence\ and expend the tree with the two branches $\grow{\abranch}{\afeat}$ and $\grow{\abranch}{\bar{\afeat}}$.
...
\begin{footnotesize}
		...
		$\dom \gets (\lambda : {2^{\prod_{\afeat \in \features}\{\afeat,\bar{\afeat}\}}} \mapsto \features)$\;
		$\best \gets (\lambda : {2^{\prod_{\afeat \in \features}\{\afeat,\bar{\afeat}\}}} \mapsto \infty)$\;
		...
		\lnl{line:dive}\eIf{$\bud \neq \emptyset$}{
			...
			\lnl{line:branching}\lForEach{$v \in \{\afeat, \bar{\afeat}\}$}{
						$\bud \gets \bud \cup \{\grow{\abranch}{v}\}$
					}
			}
		}{
...
\begin{eqnarray}
	...
	%\textrm{minimum entropy:} & \sum_{v \in \{\afeat,\bar{\afeat}\}} \frac{|\allex(\abranch \wedge v)|}{|\allex(\abranch)|} \cdot -\sum_{\aclass \in \{\bot,\top\}} \frac{|\setex{\aclass}(\abranch \wedge v)|}{|\setex{\aclass}(\abranch)} \log_{2} \frac{|\setex{\aclass}(\abranch \wedge v)|}{|\setex{\aclass}(\abranch)|} \\
...
The procedure $\dominated{\abranch}$ can therefore simply check, for all parent $\abranch'$ of $\abranch$ up until the root ($\emptyset$), whether $\lb{\abranch',\abranch} \geq \best[\abranch']$. As a result, a branch $\abranch$ which is guaranteed, by this reasoning, to never belong to a non-dominated tree will not be explored further.
	...
	\begin{center}
		...
		\scalebox{1}{
			\begin{forest}
				...
				[{$b:50$}
					[{$a:22$}, edge={very thick}, edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$b$}}
					 [{$e:17^*$}, edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$a$}}
					 ]
					 [{$c:7$}, edge={very thick}, edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$\bar{a}$}}
					 	[{$f:\infty$}, edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$c$}}
							[., edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$f$}}]
							[., edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$\bar{f}$}}]
						]
						[{$g:\infty$}, edge={very thick}, edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$\bar{c}$}}
							[., edge={very thick}, edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$g$}}]
							[., edge label={node[midway,fill=white,inner sep=2pt,font=\scriptsize]{$\bar{g}$}}]
					 	]
					 ]
					]
	...
	\end{center}
	\caption{\label{fig:lowerbound} Example of lower bound computation.}
	%\caption{\label{fig:searchtree} The search tree for decision trees. \dynprog explores it depth first, whereas \budalg explores branches in the order given below the leaves.}
	\end{figure}
...
% \begin{footnotesize}
% \tabcolsep=1.5pt
% \input{src/tables/factormur.tex}
% \end{footnotesize}
% \end{center}
% \caption{\label{tab:factor} Factor analysis}
% \end{table}
