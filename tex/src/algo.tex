...
The solver DL8.5 is a dynamic programming algorithm. It relies on the observtion that given a feature test, the two resulting branches are independent subproblems. Algorithm~\ref{alg:dynprog} gives a high level view of DL8.5.
...
\begin{itemize}
	...
	They are added to $\bud$ if their depth is strictly less than $\maxd$ and if they are not pure, otherwise they are marked as terminal.

\item As long as there is no bud ($\bud = \emptyset$), we pop the last assignment $(\abranch,\afeat)$ from sequence
and mark the feature $\afeat$ not available for branch $\abranch$. If there is at least one available feature for branch $\abranch$, we add $\abranch$ to $\bud$. 
Otherwise, we mark it as terminal and we 
...
% \clearpage
	...
	\begin{algorithm}
		...
		$\sequence \gets []$\;
		$\bud \gets \{\emptyset\}$\;
		$\error \gets \best \gets \min(|\negex|,|\posex|)$\;
		$\dom \gets (\lambda : {2^{\features}} \mapsto \features)$\;
		$\maxe \gets (\lambda : {2^{\features}} \mapsto \infty)$\;
		$\opt \gets (\lambda : {2^{\features}} \mapsto \fal)$\;
		
		\While{$|\sequence| + |\bud| > 0$}{
		\eIf{$\bud \neq \emptyset$}{
			$\abranch=\select{\bud}$\;
			$\afeat = \argmin_{\afeat \in \dom[\abranch]}(\error[\abranch,\afeat])$\;
			...
			\eIf{$\error[\abranch,\afeat] = 0$ or $|\abranch| = \maxd-1$} {
				...
				$\maxe[\abranch] \gets \error[\abranch,\afeat]$\;
				$\opt[\abranch] \gets \tru$\;
			}{
			...
			$\best \gets \min(\best, \error)$\;
			\Repeat{$\dom[\abranch] \neq \emptyset$}{
				pop $(\abranch,\afeat)$ from $\sequence$\;
				$\maxe[\abranch] \gets \min(\maxe[\abranch], \maxe[\abranch \wedge \afeat] +  \maxe[\abranch \wedge \bar{\afeat}])$\;
				\eIf{$\dom[\abranch] \neq \emptyset$} {
					$\bud \gets \bud \cup \{\abranch\}$\;
					\lIf{$\opt[\abranch]$}{$\error \gets \error - \maxe[\abranch]$}
				} {
					$\opt[\abranch] \gets \tru$\;
				}
			}
...
\end{normalsize}
\end{center}
\caption{\label{tab:d3} Restarts (max depth=3)}
\end{table}
...
\end{normalsize}
\end{center}
\caption{\label{tab:d4} Restarts (max depth=4)}
\end{table}
...
\end{normalsize}
\end{center}
\caption{\label{tab:d5} Restarts (max depth=5)}
\end{table}
