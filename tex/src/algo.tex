...
\medskip
		...
		\begin{theorem}
			...
			\begin{proof}
				...
				We show that this is true for Algorithm~\ref{alg:bud} by recursion on the size of the branch. There is only one branch of size $0$ (the emptyset) and it is given as initial input in \bud. Now suppose that every branch of size $s < \mdepth$ is explored (i.e., inserted into \bud), and consider an arbitrary branch $\abranch \wedge \afeat$ with $|\abranch|=s$. By the recursion hypothesis, the branch $\abranch$ has been put into \bud. Moreover, as long as $\bud \neq \emptyset$, \budalg does not stop. 
...
% \clearpage
	...
	\begin{algorithm}
		...
		\eIf{$\bud \neq \emptyset$}{
			...
			\lnl{line:leaves}\eIf{$|\abranch| = \maxd$} {
			...
			\ForEach{$v \in \{\afeat, \bar{\afeat}\}$}{
					\lnl{line:branching}\lIf{$\error[\abranch,v] > 0$}{
						$\bud \gets \bud \cup \{\abranch \wedge v\}$
					}
					\lElse{$\best[\abranch,v] = 0$}
			}
			}
		}{
