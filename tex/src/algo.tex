...
\newcommand{\abranch}[0]{\ensuremath{b}}
	...
	\SetKwFunction{greedy}{Greedy-Buddy}
...
\begin{itemize}
	...
	They are added to $\bud$ if their depth is strictly less than $\maxd$ and if they are not pure, otherwise they are terminal tests and we record the corresponding error.

\item As long as there is no bud ($\bud = \emptyset$), we pop the last assignment $(\abranch,\afeat)$ from sequence
and mark the feature $\afeat$ not available for branch $\abranch$ and update the recorded best error of its subtrees. If there is at least one available feature for branch $\abranch$, we add $\abranch$ to $\bud$. 
...
\medskip
		...
		\begin{theorem}
			...
			No proof is given, but independent subtrees are not explored in Algorithm~\ref{alg:bud} either hence both algorithms do the same computation, except not in the same order. The big difference is that whereas the cost of finding a first solution is $\Theta((n + 2^{k-1})m^{k-1})$ for Algorithm~\ref{alg:dynprog}, it is equal to $\Theta(kn + 2^k)$ for Algorithm~\ref{alg:bud}.
