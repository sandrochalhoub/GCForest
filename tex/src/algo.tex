...
\medskip
		...
		\begin{theorem}
			...
			\begin{proof}[sketch]
				...
				Now suppose that for $\mdepth \leq d$ every branch $\abranch$ explored by \dynprog (a recursive call ends on this branch) is also explored by \budalg ($\best[\abranch]$ is set in Lines~\ref{line:best1} or \ref{line:best2}), and let $\mdepth=d+1$. Without loss of generality, we can take an arbitrary branch $\abranch$ of length $d$ and show that the same extentions of $\abranch$ are explored by both algorithms.
	...
	\begin{algorithm}
		...
		\eIf{$\bud \neq \emptyset$}{
			...
			\lnl{line:leaves}\eIf{$|\abranch| = \maxd$ or $\error[\abranch] = 0$} {
				...
				\lnl{line:best1}$\best[\abranch] \gets \error[\abranch]$\;
			}{
		...
		\scalebox{1}{
			\begin{forest}
				for tree={%
					l sep=12pt,
					s sep=3.5pt,
					node options={shape=circle, minimum width=10pt, inner sep=0pt, font=\scriptsize},
